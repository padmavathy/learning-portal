...
RSpec.describe "Quiz", type: :request do
  describe "creates quiz, accepts answer and calculates score" do
    ...
    it "accepts answer" do
      get quiz_chapter_path Chapter.first
      chosen_option = Quiz.first.question.answers.where(is_correct: true).first
    ...
    context 'scoring' do
      context 'when answer is correct' do
        it "responds with correct score" do
          ...
          expect(api_response['status']).to eq 200
          expect(api_response['score']['correct_answers']).to eq '20%'
        end
      end
      ...
      context 'when answer is wrong' do
        it "responds with correct score" do
          get quiz_chapter_path Chapter.first
          chosen_option = Quiz.order('RANDOM()').last.question.answers.where(is_correct: false).first
          patch quiz_path(Quiz.first, response: chosen_option )
         
          get score_chapter_path(Chapter.first)        
          api_response = JSON.parse(response.body)
          expect(api_response['status']).to eq 200
          expect(api_response['score']['wrong_answers']).to eq '80%'
        end
      end
      ...
      context 'when answer is skipped' do
        it "responds with correct score" do
          get quiz_chapter_path Chapter.first
          chosen_option = Quiz.order('RANDOM()').last.question.answers.where(is_correct: false)
          ...
          expect(api_response['status']).to eq 200
          expect(api_response['score']['skipped_questions']).to eq '0%'
        end
      end
      ...
      context 'when question is not seen' do
        it "responds with correct score" do
          ...
          expect(api_response['status']).to eq 200
          expect(api_response['score']['unseen_questions']).to eq '0%'
        end
      end
    end
  end
